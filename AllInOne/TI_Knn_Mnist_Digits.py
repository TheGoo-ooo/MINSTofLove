...
class KNNDigits():
    ...
    #Va définier les limites du chiffre afin d'enlever les parties inutiles
...
#        ax.imshow(image)
        ...
        rect = patches.Rectangle(rects[0][0], rects[0][1], rects[0][2], linewidth=2, edgecolor='r', facecolor='none')
        
        ax.add_patch(rect)
        ex = self.predictionKnn(nums[0])

        digits[rects[0][0][0]][rects[0][0][1]] = [ex]
#        ax.text(rects[0][0][0], rects[0][0][1] -10, str(int(ex)), fontsize=16, bbox=dict(facecolor='yellow', alpha=0.5))
