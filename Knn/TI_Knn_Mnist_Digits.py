...
PATH_IMAGE = '../ressources/image1.png'
PATH_SAV='digit_model.sav'
...
def modelTrain(x_train, y_train):
    ...
    pickle.dump(knn, open(PATH_SAV, 'wb'))

#Obtient le pourcentage de test réussis
def scoreKnn(x_test, y_test):
    # load the model from disk
    knn = pickle.load(open(PATH_SAV, 'rb'))
    list_hog_fd = []
...
def rectangleImage(image, padd):
    fig, ax = plt.subplots(1)
    ax.imshow(image)

    nums, ph, rects = inputImage(image, padd)
    digits = limitImage(rects, nums)
    for n in range(len(nums)):
        rect = patches.Rectangle(rects[n][0], rects[n][1], rects[n][2], linewidth=3, edgecolor='g',
                                 facecolor='none')
        
        ax.add_patch(rect)
        ex = predictionKnn(nums[n])
...
if __name__ == '__main__':
    ...
    if os.path.isfile(PATH_SAV) and os.access(PATH_SAV, os.R_OK):
        pass
    else:
        # Ces deux lignes sert à créer le modèle .sav qui est le résultat de l'entrainement du modèle
        modelTrain(x_train,y_train)
        im ,t= scoreKnn(x_test,y_test)
    
    rectangleImage(cv2.imread(PATH_IMAGE), 10)